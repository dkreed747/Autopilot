...
ClientIdentity makeClientIdentity(const ConsoleConfig& config) {
  ...
  if (config.sourceId.empty()) {
    ...
    identity.sourceId = arlcore::UuidFactory::getInstance().parseGuidFromString(config.sourceId);
  }
  if (config.platformId.empty()) {
    ...
    identity.platformId = arlcore::UuidFactory::getInstance().parseGuidFromString(config.platformId);
  }
  return identity;
}
...
ClientIdentity makeLocalAutonomyIdentity(const IdentityConfig& identity) {
  ...
  if (!identity.platformId.empty()) {
    clientIdentity.platformId = arlcore::UuidFactory::getInstance().parseGuidFromString(identity.platformId);
  }
